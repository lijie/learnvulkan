...
void PbrIblApp::InitScene() {
  ...
  auto sn1 = NewNode<Node>();
  sn1->mesh = 0;
  sn1->material = 0;
  sn1->materialParamters = {
...
void PbrIblApp::Update(float deltaTime) {
...
void PbrIblApp::Render(double deltaTime) {
  if (!prepared) return;
  Update(deltaTime);
  context_->Draw(&scene);
