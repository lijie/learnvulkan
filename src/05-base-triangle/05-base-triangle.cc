...
void TriangleApp::SetupVertexDescriptions() {
  ...
  vertices.bindingDescriptions[0] =
      lvk::initializers::VertexInputBindingDescription(
  ...
  vertices.attributeDescriptions[0] =
      lvk::initializers::VertexInputAttributeDescription(
...
void TriangleApp::UpdateUniformBuffers() {
  uboVS.projection =
      glm::perspective(glm::radians(45.0f), float(width / height), 0.1f, 10.0f);

  auto view =
      glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f),
                  glm::vec3(0.0f, 0.0f, 1.0f));
  uboVS.modelView = glm::rotate(glm::mat4(1.0f), glm::radians(90.0f),
                                glm::vec3(0.0f, 0.0f, 1.0f)) *
                    view;
...
void TriangleApp::SetupDescriptorSetLayout() {
  std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
      ...
      lvk::initializers::DescriptorSetLayoutBinding(
  ...
  VkDescriptorSetLayoutCreateInfo descriptorLayout =
      lvk::initializers::DescriptorSetLayoutCreateInfo(
...
void TriangleApp::PreparePipelines() {
  VkPipelineInputAssemblyStateCreateInfo inputAssemblyState =
      lvk::initializers::PipelineInputAssemblyStateCreateInfo(
          VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, 0, VK_FALSE);

  VkPipelineRasterizationStateCreateInfo rasterizationState =
      lvk::initializers::PipelineRasterizationStateCreateInfo(
          VK_POLYGON_MODE_FILL, VK_CULL_MODE_NONE,
          VK_FRONT_FACE_COUNTER_CLOCKWISE, 0);

  VkPipelineColorBlendAttachmentState blendAttachmentState =
      lvk::initializers::PipelineColorBlendAttachmentState(0xf, VK_FALSE);

  VkPipelineColorBlendStateCreateInfo colorBlendState =
      lvk::initializers::PipelineColorBlendStateCreateInfo(
          1, &blendAttachmentState);

  VkPipelineDepthStencilStateCreateInfo depthStencilState =
      lvk::initializers::PipelineDepthStencilStateCreateInfo(
          VK_TRUE, VK_TRUE, VK_COMPARE_OP_LESS_OR_EQUAL);

  VkPipelineViewportStateCreateInfo viewportState =
      lvk::initializers::PipelineViewportStateCreateInfo(1, 1, 0);

  VkPipelineMultisampleStateCreateInfo multisampleState =
      lvk::initializers::PipelineMultisampleStateCreateInfo(
          VK_SAMPLE_COUNT_1_BIT, 0);

  std::vector<VkDynamicState> dynamicStateEnables = {VK_DYNAMIC_STATE_VIEWPORT,
                                                     VK_DYNAMIC_STATE_SCISSOR};
  VkPipelineDynamicStateCreateInfo dynamicState =
      lvk::initializers::PipelineDynamicStateCreateInfo(
  ...
  shaderStages[0] =
      LoadShader(GetShadersPath() + "05-vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
  shaderStages[1] = LoadShader(GetShadersPath() + "05-frag.spv",
                               VK_SHADER_STAGE_FRAGMENT_BIT);

  VkGraphicsPipelineCreateInfo pipelineCreateInfo =
      initializers::PipelineCreateInfo(pipelineLayout, renderPass, 0);
...
void TriangleApp::SetupDescriptorSet() {
  ...
  textureDescriptor.imageView =
      texture
          .view;  // The image's view (images are never directly accessed by the
...
void TriangleApp::BuildCommandBuffers() {
  ...
  for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
    ...
    vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                           &vertexBuffer.buffer_, offsets);
    vkCmdBindIndexBuffer(drawCmdBuffers[i], indexBuffer.buffer_, 0,
