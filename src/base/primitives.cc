...
static uint32_t addPos(MeshSection &mesh, vec3f p) {
  VertexLayout v{};
  v.position = p;
...
static void addTriangle(MeshSection &mesh, uint32_t a, uint32_t b, uint32_t c) {
  mesh.indices.push_back(a);
  mesh.indices.push_back(b);
  mesh.indices.push_back(c);
}

static void addTriangle(MeshSection &mesh, vec3f a, vec3f b, vec3f c) {
...
PrimitiveMesh cube(float width /*= 1*/, float height /*= 1*/, float depth /*= 1*/) {
  ...
  for (int i = 0; i < 6; ++i) {
    auto index = static_cast<int32_t>(section.vertices.size());
    for (int j = 0; j < 4; ++j) section.vertices.push_back({pnt[cube_polygons[i][j]], nrm[i], uv[j]});
    addTriangle(section, index, index + 1, index + 2);
    addTriangle(section, index, index + 2, index + 3);
  }

  return PrimitiveMesh(section);
}
