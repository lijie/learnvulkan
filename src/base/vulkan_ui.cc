...
void VulkanUI::PrepareResources() {
  ImGuiIO& io = ImGui::GetIO();
...
#else
  const std::string filename = tools::GetAssetPath() + "Roboto-Medium.ttf";
  ...
  VulkanBuffer* stagingBuffer =
      VulkanBuffer::Create(device, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
...
void VulkanUI::PreparePipeline(const VkPipelineCache pipelineCache, const VkRenderPass renderPass,
  ...
  // SRS - if we are using dynamic rendering (i.e. renderPass null), must define color, depth and stencil attachments at
...
void VulkanUI::draw(const VkCommandBuffer commandBuffer) {
  ImDrawData* imDrawData = ImGui::GetDrawData();
...
bool VulkanUI::header(const char* caption) { return ImGui::CollapsingHeader(caption, ImGuiTreeNodeFlags_DefaultOpen); }

bool VulkanUI::checkBox(const char* caption, bool* value) {
...
bool VulkanUI::checkBox(const char* caption, int32_t* value) {
  bool val = (*value == 1);
...
bool VulkanUI::radioButton(const char* caption, bool value) {
...
bool VulkanUI::sliderFloat(const char* caption, float* value, float min, float max) {
...
bool VulkanUI::sliderInt(const char* caption, int32_t* value, int32_t min, int32_t max) {
...
bool VulkanUI::comboBox(const char* caption, int32_t* itemindex, std::vector<std::string> items) {
  if (items.empty()) {
    return false;
  }
...
bool VulkanUI::button(const char* caption) {
...
bool VulkanUI::colorPicker(const char* caption, float* color) {
...
void VulkanUI::text(const char* formatstr, ...) {
  va_list args;
